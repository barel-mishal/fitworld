...
import {
  type MergeProfileType,
  serverMergeHeight,
  serverMergeProfile,
  serverMergeWeight,
} from "../api/service_user_info/server-user-personal-info";
...
export interface AssessmentStoreType {
  ...
  actions: {
    ...
    mergeWeight: {
      submit: QRL<
        (
          this: { isRunning: boolean },
          data: { value: number; _type: string; record: string },
        ) => Promise<{ merge: [TimeSeriesData] }>
      >;
      isRunning: boolean;
    };
    mergeHeight: {
  ...
  cahngeWeightUnit: QRL<
    (
      this: { data: AssessmentStoreType["data"] },
      value: number,
      fromUnit: WeightGetter["type"],
      toUnit: WeightGetter["type"],
    ) => void
  >;
}
...
export const useAssessmentStore = (data: TypeSchemaAssessment) => {
  ...
  const assessmentStore = useStore<AssessmentStoreType>({
    ...
    cahngeWeightUnit: $(async function (
      this: { data: AssessmentStoreType["data"] },
      value: number,
      fromUnit: WeightGetter["type"],
      toUnit: WeightGetter["type"],
...
export const contextAssessmentStore =
  createContextId<AssessmentStore>("Assessment");

export const useLoaderAssessmentData = routeLoader$(async function ({
  sharedMap,
}) {
  ...
  type CustomQueryResult = [
    null,
    null,
    [Partial<HeightGetter>],
    null,
    null,
    [Partial<WeightGetter>],
  ];
