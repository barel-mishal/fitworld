...
export const useNote = <T extends NoteProps | undefined>(note: T) => {
  ...
  const dataNotes = useResource$<{ notes: NoteProps[] }>(
    async () => await serverNotes(),
  );
  if (!note) {
    return {
      dataNotes,
...
export const NotesLayoutContext =
  createContextId<NotesLayoutContextType>("NotesLayoutContext");
