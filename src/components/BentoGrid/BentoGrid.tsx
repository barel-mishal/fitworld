...
export const BentoGrid = component$(() => {
  ...
  const generateRandomGrid = $(() => {
    const filledPositions: boolean[][] = Array(totalRows.value)
      .fill(null)
      .map(() => Array(totalCols.value).fill(false));
    ...
    function canPlaceItem(
      ...
      if (col + colSpan > totalCols.value || row + rowSpan > totalRows.value)
        return false;
  ...
  return (
    <div class="flex h-screen flex-col items-center">
      <div class="mb-4 flex items-center">
        ...
        <input
          type="number"
          class="rounded border border-gray-400 p-1"
      ...
      <div class="grid h-full w-full grid-cols-8 gap-2 overflow-auto">
        {gridItems.value.map((item) => (
          <div
            key={item.id}
            class={`flex items-center justify-center rounded bg-green-500 text-2xl text-white`}
      ...
      <button
        class="mt-4 rounded bg-blue-500 p-2 text-white"
        onClick$={onClickTailwindCopyClipboard}
      >
