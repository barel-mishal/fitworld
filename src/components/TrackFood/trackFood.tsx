...
import {
  type Ingredient,
  serverGetIngredients,
} from "~/routes/api/service_food_group/get_food_groups";
import {
  type Eat,
  transformEat,
  serverAddEat,
} from "~/routes/api/service_food_group/add_eat";
...
export const MainTrackFood = component$(() => {
  ...
  const onPressArrowsKeys = $(async (e: KeyboardEvent) => {
    ...
    if (myEats.store.state === "ingredients" && e.key === "ArrowDown") {
      ...
      const index = myEats.store.selectedFood.units.findIndex(
        (unit) => unit.id === myEats.store.eating.measurementId,
      );
  ...
  return (
    <>
      <div class="flex flex-col justify-between">
        <div class="sticky top-0 z-50 grid gap-3 bg-gray-950">
          <FieldsForNewEat />
...
export const FieldsForNewEat = component$(() => {
  ...
  return (
    <div class="sticky top-0 z-50 bg-gray-950 pb-2">
      <section class="grid grid-cols-3 gap-3">
        <label for="new-food" class="text-gray-100">
          Food
        </label>
        <label for="new-unit" class="text-gray-100">
          Unit
        </label>
        <label for="new-amount" class="text-gray-100">
          Amount
        </label>
      </section>
      <fieldset class="grid grid-cols-3 gap-3">
        ...
        <input
          ...
          onFocus$={onFocusUnit}
          class="inp"
          value={myEats.store.eating.measurement}
          onInput$={(e, el) => myEats.store.bindEating("measurement", el.value)}
        />
        <input
          id={"new-amount"}
          type="text"
          ...
          value={myEats.store.eating.amount}
          onInput$={async (e, el) => await myEats.store.updateAmount(el.value)}
        />
      </fieldset>
    </div>
  );
    ...
    <Resource
      ...
      onResolved={(value) => {
        ...
        return (
          <>
            <h5>
              <span>{value.length} Popular foods or search for</span> "
              <span>{myEats.store.eating.food}</span>"
            </h5>
            {value.map((food) => {
              return (
                <button
                  ...
                  onClick$={() =>
                    setTimeout(async () => await onClickFood(food), 300)
                  }
                >
                  {food.name}
                </button>
...
export const FestSelections = component$(() => {
  ...
  return (
    <ul class="overflow-x-auto bg-gray-950">
      <li class="bg-gray-950">
        <h5>Fast selections</h5>
        <ul class="flex-wrap-0 flex gap-4 overflow-x-auto bg-gray-950 px-1 py-2">
          <li>
            <button
              class="btn"
              onClick$={async () => {
    ...
    <>
      {!isConfrim.value && (
        <div class="grid grid-cols-[1fr,auto] gap-3">
          <button
            onClick$={() =>
              setTimeout(async () => await myEats.onClickNext(), 300)
            }
            class="btn-primary"
          >
            NEXT
          ...
          >
            <PhFlag
              class={cn(
                "absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transform transition-all ease-in-out active:rotate-6",
                !myEats.disableFinishButton.value
                  ? "fill-gray-100"
                  : "fill-gray-800",
              )}
            />
        ...
        <div class="grid grid-cols-[auto,1fr] gap-2">
          ...
          <AppLinkGlobal
            ...
            class={[
              ...
              myEats.disableFinishButton.value && "pointer-events-none",
            ]}
          >
            <PhFlag
              class={cn(
                "scale-75 transform transition-all ease-in-out active:rotate-6",
                "fill-gray-100",
              )}
            />
            <p>Finish</p>
...
export function useTrackFood() {
  ...
  const store = useStore({
    ...
    updateIngredient: $(function (
      this: { selectedFood: Ingredient; eating: Eating },
      food: Ingredient,
    ) {
    ...
    updarteUnit: $(async function (
      this: {
        selectedFood: Ingredient;
        eating: Eating;
        bindEating: QRL<
          (this: { eating: Eating }, key: keyof Eating, value: string) => void
        >;
      },
      unit: Ingredient["units"][number],
    ) {
      const u =
        this.selectedFood.units_names[this.selectedFood.units.indexOf(unit)];
    ...
    updateAmount: $(function (this: { eating: Eating }, amount: string) {
      ...
      this.eating.amount = parsedValue.success
        ? parsedValue.data
        : this.eating.amount;
    }),
  ...
  const resourceIngredients = useResource$(async ({ track, cleanup }) => {
    const value = track(() => ({
      isIngredientState: store.state,
      food: store.eating.food,
    }));
  ...
  const onClickNext = $(async () => {
    ...
    switch (goTo) {
      case "ingredients":
        ...
        break;
      case "units":
        const food = (await resourceIngredients.value).find(
          (food) => food.id === store.eating.foodId,
        );
        if (!food) return;
  ...
  return (
    <>
      {myEats.store.selectedFood && myEats.store.state === "units" && (
        <>
          <h5>{myEats.store.selectedFood.name}</h5>
              ...
              return (
                <li key={unit.id} class=" ">
                  <button
                    ...
                    class={cn("btn btn-data-active flex gap-1", "p-2")}
                    onClick$={() =>
                      setTimeout(async () => await onClickUnit(unit), 300)
                    }
                  >
...
export const contextFoodTrack =
  createContextId<ReturnType<typeof useTrackFood>>("foodTrack");
